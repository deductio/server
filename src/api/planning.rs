...
pub mod objective {
    ...
    use crate::model::objective::ObjectiveSatisfierSearchResult;
    ...
    #[put("/", data = "<data>")]
    pub async fn create_objective(user: AuthenticatedUser, data: Form<ObjectiveCreation>, mut conn: Connection<Db>) -> DeductResult<Json<Objective>> {
        Ok(Json(Objective::create(user, data.title.clone(), data.description.clone(), &mut conn).await?))
    }

    #[get("/search?<id>")]
    pub async fn get_satisfied_graphs(_user: AuthenticatedUser, id: i64, mut conn: Connection<Db>) -> DeductResult<Json<Vec<ObjectiveSatisfierSearchResult>>> {
        Ok(Json(Objective::get_satisfied_graphs(id, &mut conn).await?))
    }

    #[post("/", data = "<form>")]
    pub async fn search_objectives(_user: AuthenticatedUser, form: Form<ObjectiveSearch>, mut conn: Connection<Db>) -> DeductResult<Json<Vec<Objective>>> {
        Ok(Json(Objective::search_objectives(form.search.clone(), form.offset, &mut conn).await?))
    }
}
